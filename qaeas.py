...
def calculate_lpips_batch(img1, img2, lpips_model):
    ...
    return lpips_val
def train_standard_model(model, train_loader, val_loader, model_name, epochs=300):
    ...
    return model
def train_robust_model_pgdat(model, train_loader, val_loader, model_name, epochs=300):
    ...
    return model
def train_robust_model_trades(model, train_loader, val_loader, model_name, epochs=300, beta=6.0):
    ...
    return model
def train_robust_model_mart(model, train_loader, val_loader, model_name, epochs=300):
    ...
    return model
def train_robust_model_awp(model, train_loader, val_loader, model_name, epochs=300):
...
def run_complete_experimental_pipeline():
    ...
    for model_name, model_instance in model_configs:
        ...
        if os.path.exists(model_path):
            ...
            print(f"Training {model_name}...")
            models_c10[model_name] = train_standard_model(model_instance.to(device), train_loader_10, val_loader_10, model_name.lower().replace('-', '_') + '_cifar10', epochs=300)
    models_c100 = {}
    ...
    for model_name, model_instance in model_configs_c100:
        ...
        if os.path.exists(model_path):
            ...
            models_c100[model_name] = train_standard_model(model_instance.to(device), train_loader_100, val_loader_100, model_name.lower().replace('-', '_') + '_cifar100', epochs=300)
    print("\n" + "="*100)
    ...
    for target_name, target_model in transfer_targets.items():
        ...
        target_model = train_standard_model(target_model, train_loader_10, val_loader_10, f"transfer_{target_name}", epochs=300)
        target_model.eval()
    for seed in SEEDS:
        set_seed(seed)
