...
class ResNet20Trainer:
    def __init__(self, train_loader, val_loader, test_loader,
        ...
        self.device = device
        self.model = ResNet20(num_classes=10).to(device)

        if run_name is None:
            timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
            self.run_name = f"resnet20_run_{timestamp}"
        else:
            self.run_name = run_name

        self.target_model_dir = os.path.join('target_model_1')
        self.target_result_dir = os.path.join('target model 1 training result')
    ...
    def train(self, epochs=200, lr=0.05, weight_decay=5e-4, patience=20,
              scheduler_type='plateau', save_every=5, mixup_alpha=0.2):
        ...
        for epoch in epoch_bar:
            ...
            if (epoch + 1) % save_every == 0:
                self.save_model(f'resnet20_epoch_{epoch+1}.pth')

            if val_acc >= self.metrics_tracker.best_metrics['best_val_acc']:
                self.save_model('resnet20_best.pth')
        ...
        self.load_model('resnet20_best.pth')
    ...
    def save_model(self, filename='resnet20.pth'):
    ...
    def load_model(self, filename='resnet20_best.pth'):
        model_path = os.path.join(self.target_model_dir, filename)
...
def train_robust_resnet20(train_loader, val_loader, test_loader, run_name=None, **kwargs):
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print(f"Using device: {device}")

    trainer = ResNet20Trainer(train_loader, val_loader, test_loader, device, run_name)
    ...
    print(f"Model saved at: {os.path.join(trainer.target_model_dir, 'resnet20_best.pth')}")
    print(f"Training results saved at: {trainer.target_result_dir}")

    return trained_model


def train_target_model1_resnet20(train_loader, val_loader, test_loader, run_name, **kwargs):
    model = train_robust_resnet20(train_loader, val_loader, test_loader, run_name, **kwargs)
